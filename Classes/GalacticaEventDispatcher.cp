// ===========================================================================
//	LEventDispatcher.cp			PowerPlant 2.2.1	©1995-2001 Metrowerks Inc.
// ===========================================================================
//
	...
	switch (thePart) {
		...
		case inProxyIcon:
			...
			if (macWindowP != ::FrontWindow()) {
				...
				// ERZ mod, check to see if we clicked outside the front *modal*
				// window, since the tutorial window could be in front
				LWindow *frontW = UDesktop::FetchTopModal();
				if (frontW && (frontW->GetMacWindow() == macWindowP)) {
					// actually, the targeted window is the front modal window
					frontW = nil;	// so just forget the whole thing
				}
			//	LWindow *frontW = LWindow::FetchWindowObject(::FrontWindow());
				// end ERZ mod
				if ( (frontW != nil) &&
