// ===========================================================================
//	UWMgr20Desktop.cp			PowerPlant 2.2		©1993-2000 Metrowerks Inc.
	...
	WindowPtr	inBehind)
{
	...
	if (inWindow->HasAttribute(windAttr_Modal)) {
	  #if TUTORIAL_SUPPORT // ERZ mod
	    if (Tutorial::TutorialIsActive() || (inWINDid == window_HostNewGame) || (inWINDid == window_NewGame) ) {
	        windClass = kDocumentWindowClass; // move below the floater for the moment
	    } else
	  #endif
...
#warning TODO: do endian swapping
	...
	WindowPtr	windowP;
		...
		// ERZ mod, treat altDBoxProc same as plainDBox under Carbon
		if (theWINDInfo->procID == altDBoxProc) {
		    theWINDInfo->procID = plainDBox;
		}
		// end ERZ mod
...
{										// Do nothing. WMgr 2.0 maintains
    ...
    if (Tutorial::TutorialIsActive()) {
        Tutorial* tut = Tutorial::GetTutorial();
        if (tut) {
    	    LWindow* wind = tut->GetTutorialWindow();
    	    SelectDeskWindow(wind); // tutorial always goes on top
	    }
	}
  #endif //TUTORIAL_SUPPORT
	// end ERZ mod
